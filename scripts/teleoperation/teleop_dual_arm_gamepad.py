    ...
    while simulation_app.is_running():
        try:
            ...
            if teleoperation_active:
                ...
                # Original controller action is 6 values:
                # [x, y, z, roll, pitch, yaw]
                base_action = action.clone()

                # Create 12 values from PS controller logic
                # first 6 = "virtual arm 1"
                # second 6 = "virtual arm 2"
                virtual_arm_1 = base_action.clone()
                virtual_arm_2 = base_action.clone()

                # Make virtual arm 2 rotation different
                virtual_arm_2[:, 3] = -virtual_arm_1[:, 3]  # roll opposite
                virtual_arm_2[:, 4] = virtual_arm_1[:, 4]   # pitch same
                virtual_arm_2[:, 5] = -virtual_arm_1[:, 5]  # yaw opposite

                # This is your 12-value PS-controller command
                controller_12_values = torch.cat(
                    [virtual_arm_1, virtual_arm_2],
                    dim=-1
                )

                # Print to verify
                print("Controller 12 values:", controller_12_values)
                print("Controller 12 shape:", controller_12_values.shape)

                # IMPORTANT:
                # Your Isaac task accepts only 6 values.
                # So map 12 values back into 6 values for the one simulated arm.

                # Option A: control simulated arm using virtual arm 1
                one_arm_action = controller_12_values[:, 0:6]

                # Option B: control simulated arm using virtual arm 2
                # one_arm_action = controller_12_values[:, 6:12]

                # Option C: combine both virtual arms into one command
                # one_arm_action = 0.5 * (
                #     controller_12_values[:, 0:6] +
                #     controller_12_values[:, 6:12]
                # )

                actions = one_arm_action.repeat(env.num_envs, 1)
                env.step(actions)
